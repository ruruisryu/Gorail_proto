...
namespace Game.UI
{
    ...
    public class GroundSceneManager : MonoBehaviour
    {
        ...
        void OnGameTimeChanged(int day, int hour, int minute)
        {
            ...
            float limit = GameCore.Instance?.SceneConfig?.forceExitMinutes ?? 30f;
        ...
        void ReturnToSubway(bool forced)
        {
            ...
            ScreenFader.Instance?.Fade(onBlack: () =>
            {
                ...
                float perMin = core.SceneConfig?.trackerStepsPerOutsideMinute ?? 1f;
                int steps = Mathf.RoundToInt(stayMin * perMin);
                if (steps > 0 && core.Trackers != null) core.Trackers.AdvanceAll(steps);
                Debug.Log($"[Ground] 복귀({logTag}) 체류 {stayMin}분(게임) → 추격 {steps}스텝");
