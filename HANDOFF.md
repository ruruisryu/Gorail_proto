...
- **하차 즉시 재충원**: 검문 통과로 추격자를 없애도 하차 스폰(§5-1)이 곧 상한까지 재충원 → 압박이 빨리 회복. 스펙대로지만 통과율과 함께 튜닝 포인트.
...
- **H6 연출·그래픽 폴리싱** — 부드러운 이동 글라이드, 추격자 트레일, 최근접 강조, 검문 깜빡임, 디밍, 마커 펄스, 둥근 선/글로우 등. (시각 확인 필요라 보류)
- **H1 잔여** — ChaseEvents 이벤트 허브, 상태 HUD(TMP 오버레이).
...
### 다음 후보
1. **튜닝(§16)** — `ChaseConfig`/`SceneConfig`의 [미확정] 수치를 Play로 확정(n·m, 체증 곡선, 노선당 상한표, 통과율, 첫스폰 범위, 명성 증감·감소).
2. **H6 그래픽 폴리싱** — 추격 가독성/연출(`ChaseMetrics.NearestTracker` 재사용 가능).
3. **상태 HUD(H1 잔여)** — 명성·수배도·최근접 거리 상시 표시.
